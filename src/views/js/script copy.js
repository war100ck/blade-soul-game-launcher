...
function saveConfiguration() {
  ...
  try {
    ...
    toastr.success('Configuration saved'); // Уведомление при успешном сохранении
  } catch (error) {
...
function saveRegistrationData(nickname) {
  // Используем process.cwd() для получения правильного пути к файлу в собранном приложении
  const filePath = path.join(process.cwd(), 'data', 'data.json');
  ...
  try {
    ...
    toastr.success('Данные регистрации сохранены'); // Уведомление при успешном сохранении
  } catch (error) {
    console.error('Ошибка при сохранении данных регистрации:', error);
    toastr.error('Не удалось сохранить данные регистрации'); // Уведомление при ошибке сохранения
  }
}
...
document.getElementById('saveConfigButton').addEventListener('click', () => {
  saveConfiguration(); // Уведомление будет показано только при нажатии кнопки
});
...
async function loadConfig() {
  const filePath = path.join(process.cwd(), 'data', 'config.json'); // Полный путь к файлу

  try {
    // Чтение файла с помощью fs.promises
    const data = await fs.promises.readFile(filePath, 'utf8');
    const config = JSON.parse(data); // Парсим JSON

    // Применяем загруженные данные к элементам формы
    document.getElementById('architecture32').checked = config.architecture === 'x32';
    document.getElementById('architecture64').checked = config.architecture === 'x64';
    serverIp = config.serverIp || '127.0.0.1'; // Присваиваем значение serverIp из конфигурации
    document.getElementById('serverIp').value = serverIp; // Отображаем IP в поле ввода
    
    toastr.success('Configuration loaded successfully!'); // Уведомление об успешной загрузке
  } catch (error) {
    console.error('Ошибка при загрузке конфигурации:', error);
    toastr.error('Ошибка при загрузке конфигурации: ' + error.message); // Уведомление об ошибке
  }
}
  ...
  registrationButtonSubmit.addEventListener('click', async function () {
    ...
    try {
      ...
      toastr.error('There was an error registering. Please try again later.');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('loginButtonSubmit').addEventListener('click', async function () {
    ...
    try {
      ...
      toastr.error('There was an error logging in. Please try again later.');
      console.error('Ошибка:', error);
    }
  });
